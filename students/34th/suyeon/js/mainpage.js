...
commentButton.addEventListener('click',() => {
  const myUsername = document.querySelector('#myId')   //댓글의 아이디가 될 부분
  const commentParent = document.querySelector('.usercomment')   //리스트의 부모 요소가 될 ul을 가져왔다.
  //ul 하위 내용(content)에 innerHTML으로 접근하고 수정합니다.
  commentParent.innerHTML += `<li class="eachUserComment">
    <span>${myUsername.innerText}</span>
    <p>${mineCommentInput.value}</p>
    <button><i class="fa-regular fa-heart"></i></button>
  </li>`
  mineCommentInput.value = "" // 댓글 창의 내용이 비워져야 하므로 추가했습니다.
})

//만약, 댓글이 교체되는게 아닌 밑에 계속 추가 하고 싶다면 "commentParent.innerHTML +=" 으로 교체해보자,,,
